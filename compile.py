...
def compilePosts(targetDir, rootDir, distDir):
    ...
    if not os.path.isdir(rootDir): sys.exit(rootDir + " is not an existing directory.")
    ...
    for filename in os.listdir(rootDir + os.sep + targetDir):
        ...
        if ext == ".md":
            with open(distDir + os.sep + basename + ".html", "w") as distFile:
                distFile.write(genPostHTML(rootDir + os.sep + targetDir + os.sep + filename))
        else:
            shutil.copyfile(rootDir + os.sep + targetDir + os.sep + filename, distDir + os.sep + filename)
...
def genPostHTML(target):
    ...
    if "description" in metadata: html += '<meta property="og:description" content="'+metadata["description"]+'">'
    if "cover-image" in metadata: html += '<meta property="og:image" content="'+metadata["cover-image"]+'">'

    if "title-font" in metadata:
        font = locateFile(metadata["title-font"], target_dir)
        html += '<style>@font-face{font-family:"TitleFont";src:url("'+getRelativePath(target_dir, font)+'");} h1,h2,h3,h4,h5,h6{font-family: "TitleFont", serif;}</style>'

    if "text-font" in metadata:
        font = locateFile(metadata['text-font'], target_dir)
        html += '<style>@font-face{font-family:"TextFont";src:url("'+getRelativePath(target_dir, font)+'");} body{font-family: "TextFont", sans-serif;}</style>'

    if "style-file" in metadata:
        cssFile = locateFile(metadata["style-file"], target_dir)
    else:
        cssFile = locateFile("default.css", target_dir)
    html += '<link rel="stylesheet" href="'+getRelativePath(target_dir, cssFile)+'"/>'

    html += '</head><body>'

    if "cover-image" in metadata: html += genCoverImageHTML(metadata)
    html += '<main><article>'
